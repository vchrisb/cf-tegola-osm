...
DB_NAME=$(echo $VCAP_SERVICES | jq -c -r '.["aws-rds-postgres"] | .[0].credentials.database')
DB_HOST=$(echo $VCAP_SERVICES | jq -c -r '.["aws-rds-postgres"] | .[0].credentials.hostname')
DB_PORT=$(echo $VCAP_SERVICES | jq -c -r '.["aws-rds-postgres"] | .[0].credentials.port')
DB_USER=$(echo $VCAP_SERVICES | jq -c -r '.["aws-rds-postgres"] | .[0].credentials.username')
DB_PW=$(echo $VCAP_SERVICES | jq -c -r '.["aws-rds-postgres"] | .[0].credentials.password')
